...
export default function ToolsShowcase() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // When user scrolls down from top section into cards section:
      // Cards STAY TOGETHER in tilted fan layout.
      // ONLY when user scrolls DEEPER down towards the bottom (rect.top < -250px):
      // Cards un-tilt and separate into spaced-out horizontal row!
                      ...
                      >
                        <div className="space-y-1.5">
                          {tool.features.map((feat, idx) => (
                            <div
                              key={idx}
                              className="flex items-center gap-2 text-[10.5px] sm:text-[11px] text-slate-700 font-sans leading-snug"
                            >
